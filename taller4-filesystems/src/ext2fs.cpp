...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number)
{
	...
	Ext2FSBlockGroupDescriptor* desc =  block_group(blockgroup_number);

	unsigned char* buffer = (unsigned char*)malloc(sizeof(Ext2FSInode));
	unsigned char* buffer_inodo;
	read_block(desc->inode_table + _superblock. *offset, buffer);

	for(int i = 0; i < sizeof(Ext2FSInode)){
		buffer_inodo[i] = buffer[]
	}
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	if (block_number< 12){
		...
		if(block_number < 268){
			...
			unsigned int res = ((uint*)block_buf)[block_number-12];

			free(block_buf);

			return(res);
		} else {
			unsigned int double_block_address = inode->block[13];
			unsigned char* double_block_buf = (unsigned char*)malloc(1024<<_superblock->log_block_size);
			read_block(double_block_address, double_block_buf);

			block_number = block_number-268;
			unsigned int index = block_number/256;
			unsigned int simple_block_address = ((uint*)double_block_buf)[index];
			unsigned char* simple_block_buf = (unsigned char*)malloc(1024<<_superblock->log_block_size);
			read_block(simple_block_address,simple_block_buf);
			unsigned int res = ((uint*)simple_block_buf)[block_number% 256];
			
			free(double_block_buf);
			free(simple_block_buf);
